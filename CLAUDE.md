...
- If the system auto-mounts the SSD before the backup runs (e.g., a desktop session is active), the LUKS mapper will have a different name (e.g., `luks-6f57da7c-...`) and a different mount point. The backup script detects this via `lsblk` and uses whatever mount is active — `BACKUP_DEST` is set dynamically, not hardcoded. Note this detection runs **once, at startup** — see the USB re-enumeration note below for what happens when the device changes mid-run.
- **USB re-enumeration mid-run is survivable but must be detected.** On 2026-08-10 the SSD dropped off the bus ~3 minutes into a run and came back as a new device: it had been unlocked as `encrypted_ssd` on `sda1`, and reappeared as `sdb1` under `luks-6f57da7c-...`, auto-mounted by udisks2 at a second mount point. The original mount stayed listed in `/proc/mounts` with its backing device gone, so every read returned EIO and ext4 flagged it `emergency_ro`. rsync had already finished, but rclone spent ~10 hours retrying against the dead mount and logged 5,240 errors before failing. Data was never at risk — rclone's `not deleting files as there were IO errors` guard kept the EIO reads from being read as deletions, so S3 was left stale rather than damaged.
  - `dest_alive()` guards against this. It **touches the filesystem** (creates and removes `.backup-alive` at the mount root) rather than calling `mountpoint(1)` — a mount whose device has vanished still passes a mountpoint check, which is exactly why nothing noticed for 10 hours. The probe file lives outside `backups/` so rclone never syncs it.
  - It runs at two checkpoints (before rsync, and between rsync and the S3 sync) and then once a minute from a watchdog loop running alongside rclone, which kills the sync as soon as the source stops responding.
  - The failure notification for this case is distinct: the SSD copy completes normally and only S3 is left stale, so the message says so instead of the generic "Backup failed".
  - **Root cause is unaddressed.** The SSD sits behind an external USB3.2 hub (`2-4`) with `power/control=auto` and `autosuspend_delay_ms=0`, while the drive draws up to 896mA. The drive itself has autosuspend disabled, but the hub does not. Either move the SSD to a direct laptop port or pin the hub with a udev rule setting `power/control=on`. The watchdog limits the damage; it does not prevent the drop.
- **~900 `403 Forbidden` errors per S3 sync are pre-existing and currently unexplained.** Every run — including the successful ones — logs several hundred `Failed to copy: Forbidden`, which rclone's retry rounds absorb (`Attempt 2/3 succeeded`). They inflate every run's wall-clock and mask real failures. Worth auditing the `usb-backup-role` policy against what rclone actually issues; not yet investigated.
